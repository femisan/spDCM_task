function [out,changed,nV]=conn_importvol2bids(filename,nsub,nses,fmod,ftype,docopy,dodisp)
...
if docopy
    ...
    switch(fmod) % initializes CONN_x functional/structural fields
        case 'func'
            if ~nset, 
                [CONN_x.Setup.functional{nsub(end)}{nses(end)},nV]=conn_file(out); % note: when nsubs = [nsubs1 nsubs2] or nses = [nses1 nses2] file_bids(nsub1,nses1) is assigned to nsub2,nses2 (in order to allow subject- and session- independent data)
            else 
