...
if ~nargout
    for n=find(~okpath(:)')
        ...
        if isempty(foldername)||strcmp(foldername,pwd), fprintf('Warning: %s overloaded by version in current folder (%s). If experiencing unexpected errors please try cd''ing to a different folder before starting CONN, or removing the duplicated file %s from the folder %s\n',files{n},pwd,files{n},pwd);
        else fprintf('Warning: %s overloaded by version in folder %s. If experiencing unexpected errors please try deleting the folder %s from the Matlab path (see ''pathtool'')\n',files{n},foldername,foldername);
        end
    end
end
files=files(~okpath);
