...
%     default_mniphase                      : same as default_mni but with vdm/phasemap information
%     default_ss                            : default subject-space preprocessing pipeline
%     default_ssphase                       : same as default_ss but with vdm/phasemap information
...
steps_descr={{'INPUT: structural&functional volumes','OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Gray/White/CSF normalized masks, realigned slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&VDM volumes','OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Gray/White/CSF normalized masks, realigned&unwarp slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Gray/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&VDM volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Gray/White/CSF masks, realigned&unwarp slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Gray/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},...
    ...
    {'INPUT: functional volumes','OUTPUT: subset of functional volumes; cropped first-level covariates (if already defined)'},{'INPUT: functional volumes','OUTPUT: functional volumes (same files re-oriented, not resliced)'},{'INPUT: functional volumes','OUTPUT: functional volumes (same files translated, not resliced)'},{'INPUT: structural and functional volumes','OUTPUT: functional volumes (same files translated, not resliced)'}, ...
...
if ~nargin||isempty(STEPS)||dogui,
    ...
    if any(cellfun('length',regexp(dlg.steps_names,'^functional'))), dlg.m10=uicontrol('style','popupmenu','units','norm','position',[.05,.13,.3,.05],'value',1+sets,'string',[{'Process primary dataset'},arrayfun(@(n)sprintf('Process secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0)],'tooltipstring','<HTML>Apply this preprocessing to selected dataset in your curent CONN project (the same dataset will hold the output files)<br/> - note: only when preprocessing the primary dataset (dataset-0) non-essential preprocessing outputs (ROIs, first- and <br/>second- level covariates) will be automatically imported into your CONN project</HTML>','visible','on','fontsize',9+font_offset);
    else dlg.m10=[];
    end
    ...
    if multiplesteps
        ...
        dlg.m8a=uicontrol('style','pushbutton','units','norm','position',[.84,.87,.11,.05],'string','Add','fontweight','bold','tooltipstring','Adds new data preprocessing step to this list','callback',@conn_setup_preproc_update_add,'fontsize',9+font_offset);
        dlg.m8b=uicontrol('style','pushbutton','units','norm','position',[.84,.82,.11,.05],'string','Remove','tooltipstring','Removes selected preprocessing step from this list','callback','dlg=get(gcbo,''userdata''); str=get(dlg.m7,''string''); str=str(setdiff(1:numel(str),get(dlg.m7,''value''))); set(dlg.m7,''string'',str,''value'',[]); feval(get(dlg.m0,''callback'')); ','fontsize',9+font_offset);
        dlg.m8c=uicontrol('style','pushbutton','units','norm','position',[.84,.77,.11,.05],'string','Move up','tooltipstring','Moves selected preprocessing step up in this list','callback','dlg=get(gcbo,''userdata''); str=get(dlg.m7,''string''); val=get(dlg.m7,''value''); idx=1:numel(str); idx(val)=min(idx(val))-1.5; [nill,idx]=sort(idx); str=str(idx); set(dlg.m7,''string'',str,''value'',find(rem(nill,1)~=0));','fontsize',9+font_offset);
        dlg.m8d=uicontrol('style','pushbutton','units','norm','position',[.84,.72,.11,.05],'string','Move down','tooltipstring','Moves selected preprocessing step down this list','callback','dlg=get(gcbo,''userdata''); str=get(dlg.m7,''string''); val=get(dlg.m7,''value''); idx=1:numel(str); idx(val)=max(idx(val))+1.5; [nill,idx]=sort(idx); str=str(idx); set(dlg.m7,''string'',str,''value'',find(rem(nill,1)~=0));','fontsize',9+font_offset);
        dlg.m8e=uicontrol('style','pushbutton','units','norm','position',[.84,.67,.11,.05],'string','Save','tooltipstring','Saves this data preprocessing pipeline list for future use','callback',@conn_setup_preproc_save,'fontsize',9+font_offset);
        dlg.m8f=uicontrol('style','pushbutton','units','norm','position',[.84,.62,.11,.05],'string','Load','tooltipstring','Loads data preprocessing pipeline list from file','callback',@conn_setup_preproc_load,'fontsize',9+font_offset);
...
if any(ismember({'structural_manualspatialdef','functional_manualspatialdef'},lSTEPS))
    if isempty(respatialdef)
        ...
        for ntime=1:ntimes
            if DOSPM12, 
                str='Select deformation field volume (e.g. y_*.nii)'; disp(str);
                [tfilename1,tfilename2]=uigetfile('*.nii',str);
            else
                str='Select transformation file (e.g. *_seg_sn.mat)'; disp(str);
                [tfilename1,tfilename2]=uigetfile('*.mat',str);
            end
            if ~ischar(tfilename1), return; end
            filename=fullfile(tfilename2,tfilename1);
...
if parallel_N>0,
    ...
    info=conn_jobmanager('submit','setup_preprocessing',subjects,parallel_N,[],...
        STEPS,...
        'sessions',sessions,'sets',sets,'fwhm',fwhm,'sliceorder',sliceorder,'ta',ta,'unwarp',unwarp,'removescans',removescans,'applytofunctional',applytofunctional,...
    ...
    CONN_x.SetupPreproc.log{end+1}={'timestamp',datestr(now),...
        STEPS,...
        'subjects',subjects,'sessions',sessions,'sets',sets,'fwhm',fwhm,'sliceorder',sliceorder,'sliceorder_select',sliceorder_select,'ta',ta,'unwarp',unwarp,'removescans',removescans,'applytofunctional',applytofunctional,...
        'coregtomean',coregtomean,'coregsource',coregsource,'reorient',reorient,'respatialdef',respatialdef,'art_thresholds',art_thresholds,'voxelsize_anat',voxelsize_anat,'voxelsize_func',voxelsize_func,'boundingbox',boundingbox,'interp',interp,...
        'doimport',doimport,'dogui',0,'functional_template',functional_template,'structural_template',structural_template,...
        'affreg',affreg,'tpm_template',tpm_template,'tpm_ngaus',tpm_ngaus};
end
...
for iSTEP=1:numel(STEPS)
    ...
    if strncmp(lower(STEP),'interactive_',numel('interactive_'))
        ...
        elseif any(strcmpi(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),{'functional_removescans','functional_manualorient','structural_manualorient','functional_center','functional_centertostruct','structural_center','functional_motionmask'}))
        ...
        elseif any(strcmpi(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),{'functional_removescans','functional_manualorient','structural_manualorient','structural_manualorient','functional_center','functional_centertostruct','structural_center'}))
                ...
                for isubject=1:numel(subjects),
                    ...
                    for nses=nsess(:)'
                        ...
                        if applytofunctional||strcmp(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),'functional_segment&normalize_indirect'),
                            conn_set_functional(nsubject,nses,sets,outputfiles{isubject}{nses}{5});
                        end
                    end
                end
            ...
            case 'functional_art'
                if ~sets||ALLSETSPERMISSIONS,
                    ...
                    if yok,
                        ...
                        conn_importl2covariate({'QC_ValidScans','QC_InvalidScans','QC_MaxMotion','QC_MeanMotion','QC_MaxGSchange','QC_MeanGSchange'},{y1,y2,y3,y4,y5,y6},0,subjects,{'CONN Quality Assurance: Number of valid (non-outlier) scans','CONN Quality Assurance: Number of outlier scans',['CONN Quality Assurance: Largest motion observed',str_motion],['CONN Quality Assurance: Average motion observed',str_motion],['CONN Quality Assurance: Largest global BOLD signal changes observed',str_global],['CONN Quality Assurance: Average global BOLD signal changes observed',str_global]});
                    end
                end
...
if ~isempty(dlg.m0b), 
    ...
    if ~nm7, 
        ...
        set([dlg.m8b dlg.m8c dlg.m8d],'enable','off');
    else
        ...
        set([dlg.m8b dlg.m8c dlg.m8d],'enable','on');
    end
end
...
if ~nargin, hdl=gcbo; end
dlg=get(hdl,'userdata');
val=listdlg('liststring',dlg.steps_names(dlg.steps_order),'selectionmode','single','promptstring',{'Select pipeline (in bold) or individual preprocessing step (in regular font)'},'ListSize',[700 200]);
if ~isempty(val)
