...
%                       datasets for ROI-level timeseries extraction (secondary dataset-1 and above); default 
%                       secondarydatasets=struct('functionals_type',2)
%       functionals_type       : Source of functional data: 
%                                 1: same as 'Setup.functionals' field
%                                 2: same as 'Setup.functionals' field after removing leading 's' from filename
%                                 3: other (same as 'Setup.functionals' field but using alternative filename-change rule; 
%                                       see functionals_rule below and help conn_rulebasedfilename); 
%                                 4: other (explicitly specify the functional volume files; see functionals_explicit below) [2] 
%       functionals_rule       : (for functionals_type==3 only) regexprep(filename,functionals_rule{2},functionals_rule{3}) converts 
...
%                               file extension-)    
%       functionals_explicit: (for functionals_type==4 only) functionals_explicit{nsub}{nses} char array of functional volume files
...
%      dataset       : rois.dataset(nroi) index n to Setup.secondarydatasets(n) identifying functional dataset 
    ...
    if isfield(batch.Setup,'functionals')&&~isempty(batch.Setup.functionals),
        ...
        for isub=1:numel(SUBJECTS),
            ...
            for nses=1:CONN_x.Setup.nsessions(nsub),
                if localcopy, [nill,nill,nV]=conn_importvol2bids(batch.Setup.functionals{isub}{nses},nsub,nses,'func');
                else [CONN_x.Setup.functional{nsub}{nses},nV]=conn_file(batch.Setup.functionals{isub}{nses});
                end
...
%     if isfield(batch.Setup,'roiextract_functionals'), batch.Setup.secondarydatasets.roiextract_functionals=batch.Setup.roiextract_functionals; end
    ...
    if isfield(batch.Setup,'unwarp_functionals')
        for isub=1:numel(SUBJECTS),
            ...
            for nses=1:CONN_x.Setup.nsessions(nsub),
                [CONN_x.Setup.unwarp_functional{nsub}{nses},nV]=conn_file(batch.Setup.unwarp_functionals{isub}{nses});
                %CONN_x.Setup.nscans{nsub}{nses}=nV;
            end
        end
    end
