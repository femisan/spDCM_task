    ...
    switch(varargin{3}),
        ...
        case {'find'}
            ...
            if state,
                ...
                %h.find_state=0;  % clears the results when clicking 'cancel' button
...
opt0_vals=[];
if nargin>=1&&ischar(varargin{1}), 
    [ok,iok]=ismember(varargin{1},{'*.img; *.nii; *.mgh; *.mgz; *.gz','*.img; *.nii; *.gz','*.img; *.nii; *.tal; *.mgh; *.mgz; *.annot; *.gz','*.mat; *.txt; *.par; *.1d'});
...
dlg.m9=uicontrol('style','popupmenu','units','norm','position',[.1,.75,.8,.05],'string',arrayfun(@(n)sprintf('dataset %d',n),0:numel(a.CONN_x.Setup.secondarydataset),'uni',0),'callback',@conn_filesearch_selectconn_update,'fontsize',9+font_offset,'visible','off');
