...
% conn_print(...,'-nogui',...)                  skips GUI (uses default or last-used parameters)
% conn_print(...,'-nopersistent',...)           skips storing of the chosen parameters in last-used list
    ...
    if ~isempty(mosaic_commands)
        ...
        if domosaiccrop
            ...
            for n=1:numel(a)
                ...
                cropt_idx{n,2}=max(1,sum(~cumsum(cropt))-16):size(a{n},2)-max(0,sum(~cumsum(fliplr(cropt)))-16);
            end
        end
        switch mosaic_type
            case 1, %mosaic
