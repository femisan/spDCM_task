...
if ischar(options),
    ...
    while ~isempty(optionsnow),
        ...
        switch(lower(optionsnow)),
            ...
            case 'results_nonparametric_collapse',conn_process(21,varargin{:});
...
if any(options==4) && any(CONN_x.Setup.steps([1,2,3,4])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,3]))),
    ...
    refpial=[];
    Vmask_ref=fullfile(fileparts(which(mfilename)),'utils','surf','referenceGM.nii');
	if length(CONN_x.Setup.nsessions)==1, N=CONN_x.Setup.nsessions*numel(validsubjects); else N=sum(CONN_x.Setup.nsessions(validsubjects)); end
	N=N*nrois;
	n=0;
    all_tnames={};
		...
		for nses=1:nsess,
            ...
            if any(validrois==1)
                if strcmp(lower(REDO),'yes')||isempty(old)||~isfield(old,'sampledata')||size(old.sampledata,2)<1000,%||~isfield(old,'samplexyz'),
                    try
                        [sampledata,samplexyz]=conn_rex(Vsource,Vmask_ref,'dims',1000,'roi_threshold',.25,'level','subsetvoxels','scaling',scalinglevel,'select_clusters',0,'output_type','none');
...
if any(floor(options)==9),
    ...
    if any(CONN_x.Setup.steps([1,2,4])) && any(options==9.1|options==9) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,4])))
        ...
        for ianalysis=analyses,
            ...
            else, initial=CONN_x.Analyses(ianalysis).variables.names; initial=initial(~cellfun('length',regexp(initial,'^Grey Matter$|^White Matter$|^CSF Matter$|^QA_|^QC_'))); dims={}; ders={}; fbands={}; end
            ...
            for ncondition=validconditions,
                ...
                for nsub=1:CONN_x.Setup.nsubjects,
                    ...
                    for nroi=1:nrois2,
                        ...
                        if isempty(idx)&&names2checked, disp(['Warning: Non-existing target ROI first-level data for ',names2{nroi},' subject ',num2str(nsub),'. Skipping this target ROI']); 
...
if any(options==20)
    ...
    conn_randomise(X,y,c,m,THR,THR_TYPE,SIDE,[niters BlockDistributed],simfilename,[],xyz,[],groupingsamples,false);
end
if any(options==21), % merges permutation/randomization results
    filename=varargin{1};
    ...
    if ~isempty(newfilenames)
        ...
        for n=1:numel(newfilenames)
            ...
            if ~conn_existfile(filename)
                fprintf('merging permutation results file %s\n',newfilename);
                if ispc, [ok,msg]=system(sprintf('ren "%s" "%s"',newfilename,filename));
                else [ok,msg]=system(sprintf('mv ''%s'' ''%s''',newfilename,filename));
                end
            elseif conn_existfile(newfilename)
                fprintf('merging permutation results file %s with %s\n',newfilename,filename);
                load(filename);
                ...
                save(filename,'Pthr','Pthr_type','Pthr_side','Hist_Cluster_size','Hist_Cluster_mass','Dist_Cluster_sizemax','Dist_Cluster_massmax','Hist_Seed_size','Hist_Seed_mass','Dist_Seed_sizemax','Dist_Seed_massmax','-append');
                if ispc, [ok,msg]=system(sprintf('del "%s"',newfilename));
                else [ok,msg]=system(sprintf('rm ''%s''',newfilename));
                end
            else
                fprintf('WARNING: permutation results file %s not found. Statistics may be computed from fewer permutation/randomization samples than execepted\n',newfilename);
            end
        end
    end
end
