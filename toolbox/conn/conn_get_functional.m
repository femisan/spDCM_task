...
if nset>0
    try
        ...
        if flag, % check if file exists
            filename=CONN_x.Setup.functional{nsub}{nses}{1};
            fprintf('warning: error in dataset definition for set-%d subject %d session %d. Using set-0 functional data instead\n',nset,nsub,nses);
        else
            fprintf('warning: error in dataset definition for set-%d subject %d session %d\n',nset,nsub,nses);
        end
    end
else
    filename=CONN_x.Setup.functional{nsub}{nses}{1};
