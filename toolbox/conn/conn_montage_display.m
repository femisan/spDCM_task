...
if ~isempty(state.xcov)
    ...
    hold(state.handles.haxcov,'on'); state.handles.scalecov=text(repmat(-5,[1,size(xcov,2)*2]),[min(xcov,[],1) max(xcov,[],1)],arrayfun(@num2str,[min(state.xcov,[],1) max(state.xcov,[],1)],'uni',0),'color',.35*[1 1 1],'horizontalalignment','right','parent',state.handles.haxcov); hold(state.handles.haxcov,'off');
    axis(state.handles.haxcov,'tight','off');
    if ~isempty(state.xcov_name),
        if numel(state.xcov_name)==1, 
            ht=text(1.02*size(state.xcov,1),size(state.xcov,2)/2,state.xcov_name,'parent',state.handles.haxcov); 
...
else [state.handles.haxcov,state.handles.himcov,state.handles.scalecov,state.handles.refcov]=deal([]);
end
    ...
    function out=conn_montage_display_refresh(hObject,eventdata,option,varargin)
        ...
        switch(state.style)
            ...
            case {'movie','moviereplay'}
                if ~isempty(state.xcov), set([state.handles.himcov(:)' state.handles.scalecov(:)' state.handles.refcov(:)'],'visible','on'); set(state.handles.hax,'position',[.05 .3 .90 .65]);
