...
% assigns conn/utils/surf/FreeSurferColorLUT.txt labels file to ROI
% (e.g. for aparc+aseg.mgz files)
...
for n1=1:numel(files),
    ...
    for n2=1:numel(files{n1})
        if ispc, [nill,nill]=system(sprintf('copy "%s" "%s"',file0,files{n1}{n2}));
        else     [nill,nill]=system(sprintf('cp ''%s'' ''%s''',file0,files{n1}{n2}));
        end
    end
end
