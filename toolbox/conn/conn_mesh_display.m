...
if doinit
    ...
    if nargin<6||isempty(thr), thr=nan; end
...
if doinit
    ...
    if ~isnan(THR), V=V.*(abs(V)>THR); end
    ...
    if ~isempty(trefnames)
        if numel(trefnames)>1&&numel(tfilenameSURF)==1, %3d-atlas
            ...
            if max(trefidx)==maxdata
                ...
                tfilenameSURF=num2cell(struct('filename',char(filenameSURF),'data',arrayfun(@(n)tempdata==trefidx(n),idata,'uni',0),'vol',tempvol));
            end
