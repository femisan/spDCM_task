...
elseif ~isempty(data) % conn_slice_display(datafile [,structural_file])
    ...
    state.isstat=false;
    state.isvol=false;
    V=spm_vol(char(state.structural));
...
if doinit
    ...
    if isnan(state.volthr), state.volthr=state.Srange*[2/3;1/3]; end
    ...
    if state.isvol, 
        if isnan(state.actthr), state.actthr=max(abs(state.supra(:)))/2; end; 
...
state.handles.multislice1_delta(1)=uicontrol('style','pushbutton','units','norm','position',[.825+.025*0 .845 .025 .03],'string','-','callback',{@conn_slice_display_refresh,'multislice','-',1});
state.handles.multislice1_delta(2)=uicontrol('style','pushbutton','units','norm','position',[.825+.025*1 .845 .025 .03],'string','+','callback',{@conn_slice_display_refresh,'multislice','+',1});
state.handles.multislice2_delta(1)=uicontrol('style','pushbutton','units','norm','position',[.875+.025*0 .845 .025 .03],'string','-','callback',{@conn_slice_display_refresh,'multislice','-',2});
state.handles.multislice2_delta(2)=uicontrol('style','pushbutton','units','norm','position',[.875+.025*1 .845 .025 .03],'string','+','callback',{@conn_slice_display_refresh,'multislice','+',2});
...
if state.isvol
    uicontrol('style','text','units','norm','position',[.715 .775 .10 .05],'string','Overlay threshold','horizontalalignment','right');
    state.handles.actthr=uicontrol('style','slider','units','norm','position',[.825 .775 .10 .05],'callback',{@conn_slice_display_refresh,'actthr'},'tooltipstring','Select overlay threshold');
    set(state.handles.actthr,'value',max(0,min(1, (state.actthr-state.Vrange(1))/max(eps,state.Vrange(2)-state.Vrange(1)))));
    state.handles.actthr_txt=uicontrol('style','text','units','norm','position',[.925 .775 .05 .05],'string',mat2str(state.actthr,2));
...
uicontrol('style','text','units','norm','position',[.715 .725 .10 .05],'string','Image threshold','horizontalalignment','right');
state.handles.volthr=uicontrol('style','slider','units','norm','position',[.825 .725 .10 .05],'callback',{@conn_slice_display_refresh,'volthr'},'tooltipstring','Select image threshold');
set(state.handles.volthr,'value',max(0,min(1, (state.volthr-state.Srange(1))/max(eps,state.Srange(2)-state.Srange(1)))));
state.handles.volthr_txt=uicontrol('style','text','units','norm','position',[.925 .725 .05 .05],'string',mat2str(state.volthr,2));
uicontrol('style','text','units','norm','position',[.715 .675 .10 .05],'string','Image brightness','horizontalalignment','right');
state.handles.bright=uicontrol('style','slider','units','norm','position',[.825 .675 .10 .05],'callback',{@conn_slice_display_refresh,'bright'},'tooltipstring','Select image brightness');
...
if state.isvol
    tvalues=[1 .9:-.1:.1 .05 0];
    thdl=uimenu(hc1,'Label','overlay surface on','separator','on','callback',{@conn_slice_display_refresh,'act_transparency',1},'tag','act_transparency');
    hc2=uimenu(hc1,'Label','overlay surface transparent');
    ...
    thdl=[thdl,uimenu(hc1,'Label','overlay surface off','callback',{@conn_slice_display_refresh,'act_transparency',0},'tag','act_transparency')];
...
%     set(thdl,'checked','off');set(thdl(max(1,min(numel(thdl),1+round(state.transparency*10)))),'checked','on');
    ...
    for n1={'red','jet','hot','gray','bone','cool','hsv','spring','summer','autumn','winter','random','brighter','darker','manual','color'}
    ...
    function out=conn_slice_display_refresh(hObject,eventdata,option,varargin)
        ...
        switch(option)
            ...
            case 'info',
                ...
                conn_msgbox([{'Volume:'},reshape(cellstr(state.info.structural),1,[]),{' ','Overlay surface/contour:'},reshape(cellstr(state.info.vol),1,[]),{' ','Voxel size used for display (mm):'},{mat2str(voxelsize)}],'Slice display info');
                return;
            case 'togglepointer'
...
for n=1:3,
    ...
    d3(n)=zoomout*mean(mean(x0{n}(:,:,min(size(x0{n},3),2))-x0{n}(:,:,1),1),2);
