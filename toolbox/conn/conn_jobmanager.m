...
%           cmd_rundeployed             : 1 nodes use pre-compiled CONN only
    ...
    function conn_jobmanager_settings_update(option,varargin)
        switch(option)
            ...
            case 'edit' %{'edit','editrundeployed'}
                ...
                if isempty(tstr), tstr=conn_jobmanager_checkdeployedname; set(handles.cmd_deployedfile,'string',tstr); end
...
if ~numel(files)
    ...
    set(handles.stopall,'string','Delete job','position',[.825,.20,.15,.05]);
...
%                     fprintf('.');
            ...
            case 'details'
                ...
                tfiles={info.stdlog, info.stdout, info.stderr, info.scripts};
                ...
                h.types=uicontrol(h.hfig,'style','popupmenu','units','norm','position',[.1 .90 .9 .05],'string',{'Matlab log','console output (stdout)','error output (stderr)','submission script','submission command','submission command output','status command output'},'value',1,'callback',@conn_projectmanager_update_details); %'uiresume(gcbf)');
