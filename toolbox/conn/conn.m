...
if nargin<1,
    ...
    conn_backgroundcolorA=.19*[1 1 1];                % highlight
    ...
    CONN_gui.backgroundcolorE=max(0,min(1, .5*CONN_gui.backgroundcolor+.5*[2 2 4]/6));
	...
	CONN_h.menus.m_analyses_03sub=conn_menumanager([], 'n',4,...
                                    ...
                                    'order','vertical',...
									'position',[.235+2.5*.765/4-.135/2,.955-6*.045,.135,6*.045],...%[.0,.68,.07,3*.05],...
                                    'toggle',0,...
                                    'roll',1,...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_results_03sub=conn_menumanager([], 'n',6,...
                                    ...
                                    'order','vertical',...
									'position',[.235+3.5*.765/4-.135/2,.955-6*.045,.135,6*.045],...%[.0,.68,.07,3*.05],...
                                    'toggle',0,...
                                    'roll',1,...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_setup_07f=conn_menumanager([],	'n',3,...
									...
									'help',{'Configuration settings in distributed cluster or multi-processor environments','Displays currently running, pending, or queued jobs','Displays all past or pending jobs'},...
                                    'order','vertical',...
                                    'toggle',0,...
                                    'roll',1,...
	...
	CONN_h.menus.m_setup_07c=conn_menumanager([],	'n',3,...
                                    ...
                                    'toggle',0,...
                                    'roll',1,...
									'position',[.18,.955-2.5*.045-3*.045,.15,3*.045],...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_setup_07d=conn_menumanager([],	'n',7,...
                                    ...
                                    'toggle',0,...
                                    'roll',1,...
									'position',[.18,.955-4.5*.045-7*.045,.099,7*.045],...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_setup_07g=conn_menumanager([],	'n',2,...
                                    ...
                                    'toggle',0,...
                                    'roll',1,...
									'position',[.18,.955-3.5*.045-2*.045,.099,2*.045],...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_setup_08=conn_menumanager([],	'n',1,...
									'string',{'pending jobs'},...
									'help',{'Display currently running, pending, or queued jobs'},...
                                    'order','horizontal',...
                                    'toggle',0,...
                                    'fontangle','italic',...
...
% 									'callback',{CONN_h.menus.m_results_06a} );
    ...
    try
        ...
        if (1|isunix)&&~ismac&&CONN_gui.dounixGUIbugfix, %(||ismac&&CONN_gui.domacGUIbugfix), 
	...
	switch(lower(varargin{1})),
        case 'init',
            ...
            CONN_x=struct('name',[],'filename','','gui',1,'state',0,'ver',connver,'lastver',connver,'isready',[0 0 0 0],'ispending',0,...
                ...
                'Setup',struct(...
                 ...
                 'secondarydataset',struct('functionals_type',2,'functionals_explicit',{{}},'functionals_rule',{{}}),...
                 'unwarp_functional',{{}},...
        ...
        case 'gui_calculator'
            ...
            if CONN_x.ispending, conn_menumanager(CONN_h.menus.m_setup_08,'on',1); end
            conn_calculator;
        ...
        case 'gui_jobmanager'
            ...
            else conn_menumanager(CONN_h.menus.m_setup_08,'off',1); 
            end
        ...
        case 'gui_help',
            switch(lower(varargin{2}))
                case 'url'
                    fprintf('Please visit %s if the page is not automatically loaded\n',varargin{3});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%			
        ...
        case 'gui_setup',
            ...
            if nargin<2,
                ...
                %conn_menu('frame2border',[.005,.85-8*.06,.13,8*.06]);
            ...
            switch(state),
                case 1, %basic
                    ...
                    if nargin<2,
						...
						CONN_h.menus.m_setup_00{2}=conn_menu('edit',boffset+[.2,.6,.25,.04],'Number of sessions',num2str(CONN_x.Setup.nsessions,'%1.0f '),'<HTML>Number of scanning runs or sessions per subject <br/> - enter a single number if the same number of scanning sessions were acquired for each subject, or a different number per subject otherwise (e.g. 2 2 3)<br/> - note: "sessions" (also refered as "runs") refer to a single period of data collection. Different sessions of data may be acquired in a single visit to the scanner or across different days</HTML>','conn(''gui_setup'',2);');
                ...
                case 3, %functional
                    boffset=[.04 -.015 0 0];
                    if nargin<2,
                        conn_menu('frame',boffset+[.19,.20,.45,.585],'Functional data');
						...
						CONN_h.menus.m_setup_00{4}=conn_menu('pushbutton',boffset+[.40,.60,.18,.09],'','','','conn(''gui_setup'',4);');
                        ...
                        ht=uicontrol('style','frame','units','norm','position',boffset+[.35,.61,.29,.09],'foregroundcolor',CONN_gui.backgroundcolorA,'backgroundcolor',CONN_gui.backgroundcolorA);
                        ...
                        %conn_menu('nullstr',{'No functional','data selected'});
                        CONN_h.menus.m_setup_00{14}=conn_menu('popup',boffset+[.20,.20,.25,.05],'',{'<HTML><i> - functional tools:</i></HTML>','Slice viewer','Slice viewer with anatomical overlay (QA_REG)','Slice viewer with MNI boundaries (QA_NORM)','Display functional/anatomical coregistration (SPM)','Display functional/MNI coregistration (SPM)','Display single-slice for all subjects (montage)','Display single-slice for all timepoints (movie)', 'Apply individual preprocessing step','Copy primary dataset to secondary dataset-N','Copy secondary dataset-N to primary dataset','Exchange primary dataset and dataset-N','Reassign all functional files simultaneously'},'<HTML> - <i>slice viewer</i> displays functional dataset slices<br/> - <i>slice viewer with anatomical overlay</i>displays mean functional overlaid with same-subject structural volume<br/> - <i>slice viewer with MNI boundaries</i> displays mean functional volume slices overlaid with 25% boundaries of grey matter tissue probability map in MNI space<br/> - <i>display registration</i> checks the coregistration of the selected subject functional/anatomical files <br/> - <i>preprocessing</i> runs individual preprocessing step on functional volumes (e.g. realignment, slice-timing correction, etc.)<br/> - <i>display single-slice for all subjects</i> creates a summary display showing the same slice across all subjects (slice coordinates in world-space)<br/> - <i>copy secondary dataset-N to primary dataset</i> reassigns primary dataset functional volumes (set to match the current dataset of functional volumes defined in dataset-N)<br/> - <i>reassign all functional files simultaneously</i> reassigns current dataset functional volumes using a user-generated search/replace filename rule</HTML>','conn(''gui_setup'',14);');
                        nset=1;
                        newdelete={'<HTML><i>new</i></HTML>','<HTML><i>delete</i></HTML>'}; if numel(CONN_x.Setup.secondarydataset)==1, newdelete=newdelete(1); end
                        CONN_h.menus.m_setup_00{7}=conn_menu('popupbigblue',boffset+[.191,.74,.45,.05],'',[{'primary dataset'},arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0),newdelete],'<HTML>Enter your primary functional data as <i>primary dataset</i><br/> <br/>Secondary datasets (<i>dataset #1</i> and above) may be defined as a way of keeping track of different preprocessing pipelines (e.g. subject-space vs. MNI-space functional data) <br/> or, in conjunction with dataset-specific ROIs, as a way of using simultaneously different types of ROIs (e.g. subject-space and MNI-space ROIs)<br/> <br/> Example1 (MNI-space analyses): <i>Primary dataset</i>: MNI-space spatially-smoothed data; <i>Secondary dataset #1</i>: MNI-space raw (unsmoothed) data<br/> Example2 (surface-based analyses): <i>Primary dataset</i>: subject-space raw data; <i>Secondary dataset #1</i>: MNI-space raw (unsmoothed) data (to be used with MNI-space ROIs)</HTML>','conn(''gui_setup'',7);');
                        analysistypes={sprintf('functional dataset #%d is equal to primary dataset',nset),sprintf('functional dataset #%d is derived from primary dataset: same filenames without leading ''s'' (SPM convention for unsmoothed volumes)',nset),sprintf('functional dataset #%d is derived from primary dataset: user-defined filename differences',nset),sprintf('functional dataset #%d is independent of primary dataset: explicitly select functional files',nset)};
                        CONN_h.menus.m_setup_00{6}=conn_menu('popup',boffset+[.20,.68,.43,.05],'',analysistypes,'<HTML>Define contents of secondary functional datasets (dataset #1 and above), either explicitly or in relation to the primary dataset:<br/>note1: ''same filenames without leading s'' uses SPM convention for naming spatially smoothed volumes to identify the original raw (unsmoothed) functional data from the smoothed filenames<br/>note2: ROIs may be associated with individual datasets (see <i>Setup.ROIs</i> tab); e.g. a normalized functional dataset for MNI-space ROIs vs. a coregistered functional dataset for subject-space ROIs</HTML>','conn(''gui_setup'',6);');
                        ...
                        CONN_h.menus.m_setup_00{11}=uicontrol('style','frame','units','norm','position',[.78,.10,.20,.80],'foregroundcolor',CONN_gui.backgroundcolor,'backgroundcolor',CONN_gui.backgroundcolor);
                        ...
                        switch(varargin{2}),
                            ...
                            case 6,
                                ...
                                if functionals_type==3, 
                                    ...
                                    rule=conn_rulebasedfilename(filename,0,CONN_x.Setup.secondarydataset(nset).functionals_rule,CONN_x.Setup.functional);
                                    if ~isequal(rule,0)
                            ...
                            case 7,
                                ...
                                elseif nset==numel(CONN_x.Setup.secondarydataset)+2, %remove
                                    str=arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0);
                                ...
                                newdelete={'<HTML><i>new</i></HTML>','<HTML><i>delete</i></HTML>'}; if numel(CONN_x.Setup.secondarydataset)==1, newdelete=newdelete(1); end
                                set(CONN_h.menus.m_setup_00{7},'string',[{'primary dataset'},arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0),newdelete],'value',nset+1);
                            case 14,
...
%                                             end
                                        ...
                                        for nsub=nsubs(:)'
                                            for nses=1:CONN_x.Setup.nsessions(min(length(CONN_x.Setup.nsessions),nsub))
                                                for nset=nsets(:)'
                                                    Vsource=conn_get_functional(nsub,nses,nset);
                                                    if isempty(Vsource),
                                                        fprintf('Dataset %d subject %d session %d data not found\n',nset,nsub,nses);
                                    ...
                                    case 10, % move set-0 to set-N
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        nalt=listdlg('liststring',arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0),'selectionmode','single','initialvalue',1,'promptstring',{'copy primary dataset to:','(select target set number)'},'ListSize',[200 200]);
                                        if isempty(nalt), return; end
                                        hmsg=conn_msgbox('Loading files... please wait','');
                                        err=false;
                                        bak1=CONN_x.Setup.secondarydataset(nalt).functionals_explicit;
                                        for nsub=1:CONN_x.Setup.nsubjects
                                            for nses=1:CONN_x.Setup.nsessions(min(length(CONN_x.Setup.nsessions),nsub))
                                                VsourceUnsmoothed=cellstr(CONN_x.Setup.functional{nsub}{nses}{1});
                                                ...
                                                if ~all(existunsmoothed),
                                                    ...
                                                    CONN_x.Setup.secondarydataset(nalt).functionals_explicit{nsub}{nses}=conn_file(char(VsourceUnsmoothed));
                                                end
                                            end
                                            if err, break; end
                                        end
                                        if ishandle(hmsg), delete(hmsg); end
                                        if err||strcmp(conn_questdlg(sprintf('Set-%d functional volumes successfully reassigned',nalt),'','Ok','Undo','Ok'),'Undo'), CONN_x.Setup.secondarydataset(nalt).functionals_explicit=bak1; end
                                        if err, conn_msgbox({sprintf('Error: set-%d data for subject %d session %d not found.\n',0,nsub,nses),'Operation canceled'},'',2); end
                                        if ~err, CONN_x.Setup.secondarydataset(nalt).functionals_type=4; end
                                    case 11, % move set-N to set-0
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        nalt=listdlg('liststring',arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0),'selectionmode','single','initialvalue',1,'promptstring',{'copy primary dataset from:','(select source set number)'},'ListSize',[200 200]);
                                        if isempty(nalt), return; end
                                        hmsg=conn_msgbox('Loading files... please wait','');
                                        err=false;
                                        ...
                                        for nsub=1:CONN_x.Setup.nsubjects
                                            for nses=1:CONN_x.Setup.nsessions(min(length(CONN_x.Setup.nsessions),nsub))
                                                if CONN_x.Setup.secondarydataset(nalt).functionals_type==4
                                                    VsourceUnsmoothed=cellstr(CONN_x.Setup.secondarydataset(nalt).functionals_explicit{nsub}{nses}{1});
                                                else
                                                    Vsource1=cellstr(CONN_x.Setup.functional{nsub}{nses}{1});
                                                    VsourceUnsmoothed=conn_rulebasedfilename(Vsource1,CONN_x.Setup.secondarydataset(nalt).functionals_type,CONN_x.Setup.secondarydataset(nalt).functionals_rule);
                                                end
                                                existunsmoothed=conn_existfile(VsourceUnsmoothed); %existunsmoothed=cellfun(@conn_existfile,VsourceUnsmoothed);
                                                if ~all(existunsmoothed),
                                                    fprintf('warning: set-%d data for subject %d session %d not found.\n',nalt,nsub,nses);
                                                    ...
                                                    [CONN_x.Setup.functional{nsub}{nses},nV]=conn_file(char(VsourceUnsmoothed));
                                                    CONN_x.Setup.nscans{nsub}{nses}=nV;
                                                end
                                            end
                                            if err, break; end
                                    ...
                                    case 12, % exchange set-N and set-0
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        nalt=listdlg('liststring',arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0),'selectionmode','single','initialvalue',1,'promptstring',{'Exchange primary dataset and:','(select target set number)'},'ListSize',[200 200]);
                                        if isempty(nalt), return; end
                                        hmsg=conn_msgbox('Loading files... please wait','');
                                        err=false;
                                        bak1=CONN_x.Setup.functional; bak2=CONN_x.Setup.nscans; bak3=CONN_x.Setup.secondarydataset(nalt).functionals_explicit;
                                        for nsub=1:CONN_x.Setup.nsubjects
                                            for nses=1:CONN_x.Setup.nsessions(min(length(CONN_x.Setup.nsessions),nsub))
                                                Vsource1=cellstr(CONN_x.Setup.functional{nsub}{nses}{1});
                                                if CONN_x.Setup.secondarydataset(nalt).functionals_type==4
                                                ...
                                                existunsmoothed=conn_existfile(VsourceUnsmoothed);
                                                existvsource=conn_existfile(Vsource1);
                                                if ~all(existvsource),
                                                    fprintf('warning: set-%d data for subject %d session %d not found.\n',0,nsub,nses);
                                                    err=true; 
                                                    break;
                                                elseif ~all(existunsmoothed),
                                                    fprintf('warning: set-%d data for subject %d session %d not found.\n',nalt,nsub,nses);
                                                    err=true; 
                                                    break;
                                                else
                                                    [CONN_x.Setup.functional{nsub}{nses},nV]=conn_file(char(VsourceUnsmoothed));
                                                    CONN_x.Setup.nscans{nsub}{nses}=nV;
                                                    CONN_x.Setup.secondarydataset(nalt).functionals_explicit{nsub}{nses}=conn_file(char(Vsource1));
                                                end
                                            end
                                            if err, break; end
                                        end
                                        if ishandle(hmsg), delete(hmsg); end
                                        if err||strcmp(conn_questdlg(sprintf('Set-0 and Set-%d functional volumes successfully reassigned',nalt),'','Ok','Undo','Ok'),'Undo'), CONN_x.Setup.functional=bak1; CONN_x.Setup.nscans=bak2; CONN_x.Setup.secondarydataset(nalt).functionals_explicit=bak3; end
                                        if err, conn_msgbox({'Operation canceled'},'',2); end
                                        if ~err, CONN_x.Setup.secondarydataset(nalt).functionals_type=4; end
                                    case 13, % reassign all functional
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        nset=get(CONN_h.menus.m_setup_00{7},'value')-1;
...
%                                 CONN_x.Setup.normalized=normalized;
                    ...
                    if ~nset
                        ...
                        analysistypes={sprintf('functional dataset #%d is equal to primary dataset',nset),sprintf('functional dataset #%d is derived from primary dataset: same filenames without leading ''s''',nset),sprintf('functional dataset #%d is derived from primary dataset: user-defined filename differences',nset),sprintf('functional dataset #%d is independent of primary dataset: explicitly select functional files',nset)};
...
%                                 CONN_x.Setup.normalized=normalized;
                ...
                case 4, %ROIs
                    ...
                    if nargin<2,
                        ...
                        str=[{'from primary functional dataset'},arrayfun(@(n)sprintf('from secondary dataset #%d',n),1:max(numel(CONN_x.Setup.secondarydataset),max(CONN_x.Setup.rois.unsmoothedvolumes)),'uni',0)];
						CONN_h.menus.m_setup_00{13}=conn_menu('popup',boffset+[.512,.56,.16,.03],'',str,'<HTML>source of functional data for ROI timeseries extraction<br/> - Select <b>dataset 1 or above</b> to extract ROI BOLD timeseries from any of the secondary datasets in <i>Setup.Functional</i> (default behavior; e.g. unsmoothed volumes)<br/> - Select <b>primary dataset</b> to extract ROI BOLD timeseries from the primary dataset (e.g. smoothed volumes)</HTML>','conn(''gui_setup'',13);');
...
%                                 CONN_x.Setup.normalized=normalized;
				...
				case 6, % covariates first-level
                    boffset=[.03 -.03 0 0];
					if nargin<2,
						conn_menu('frame',boffset+[.19,.19,.48,.63],'First-level covariates / timeseries');
						...
						CONN_h.menus.m_setup_00{4}=conn_menu('filesearch',[],'Select covariate files','*.mat; *.txt; *.par; *.1d','',{@conn,'gui_setup',4},'conn(''gui_setup'',5);');
							...
							case 4,
								...
								elseif size(filename,1)==1,
                                    ...
                                    for nsub=nsubs(:)', for nses=nsessall(:)',
                                            if nses<=CONN_x.Setup.nsessions(min(length(CONN_x.Setup.nsessions),nsub)), CONN_x.Setup.l1covariates.files{nsub}{nl1covariates}{nses}=fileinfo; end;
                                        end; end
                                    txt=sprintf('%d files assigned to %d sessions x %d subjects\n',size(filename,1),length(nsessall),length(nsubs));
								else 
...
%                                             txyz=data.buttondown.matdim.mat*[tx(:) ty(:) zslice+zeros(numel(tx),1) ones(numel(tx),1)]';
                                        ...
                                        if numel(varargin)>=8, nsets=varargin{8};
                                            ...
                                            nsets=listdlg('liststring',[{'primary dataset'},arrayfun(@(n)sprintf('secondary dataset #%d',n),1:numel(CONN_x.Setup.secondarydataset),'uni',0)],'selectionmode','single','initialvalue',1,'promptstring',{'Select functional dataset for display'},'ListSize',[300 200]);
                                            if isempty(nsets), return; end
                                            nsets=nsets-1;
                                        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%			
		case 'gui_preproc',
            ...
            if nargin<2,
				...
				CONN_h.menus.m_preproc_00{1}=conn_menu('listbox',boffset+[.27,.50,.125,.20],'all effects','','<HTML>List of all effects<br/> - ROI timeseries<br/> - First-level covariates<br/> - Condition/task timeseries</HTML>','conn(''gui_preproc'',1);');
                ...
                CONN_h.menus.m_preproc_00{21}=uicontrol('style','frame','units','norm','position',boffset+[.235,.50,.16,.25],'foregroundcolor',CONN_gui.backgroundcolorA,'backgroundcolor',CONN_gui.backgroundcolorA);
		...
		case 'gui_analyses',
            ...
            if state(1)==1, %SEED-TO-VOXEL or ROI-TO-ROI
                ...
                if nargin<2,
                    ...
                    conn_menu('frame',boffset+[.135,.08,.395,.75],'Seed-based connectivity analyses');%'FC ANALYSIS OPTIONS');
                    ...
                    CONN_h.menus.m_analyses_00{10}=conn_menu('popup',boffset+[.145,.68,.31,.04],'Analysis type',analysistypes,['<HTML>Choose first-level model between the seed/source BOLD timeseries and each target ROI or voxel BOLD timeseries <br/>',...
                    ...
                    CONN_h.menus.m_analyses_00{1}=conn_menu('listbox',boffset+[.36,.31,.155,.13],'all ROIs','','<HTML>List of all seeds/ROIs <br/> - this list includes all ROI timeseres and first-level covariates except those<br/> which have already been defined/used as confounds during the denoising step</HTML>','conn(''gui_analyses'',1);');
                    CONN_h.menus.m_analyses_00{2}=conn_menu('listbox',boffset+[.145,.31,.195,.13],'Seeds/Sources','','<HTML>List of seeds/ROIs to be included in this analysis  <br/> - Connectivity measures will be computed among all selected ROIs (for ROI-to-ROI analyses) and/or between the selected ROIs and all brain voxels (seed-to-voxel analyses) <br/> - Select ROIs in the <i>all ROIs</i> list and click <b> &lt </b> to add new sources to this list<br/> - Select ROIs in this list and click <b> &gt </b> to remove them from this list </HTML>','conn(''gui_analyses'',2);');
                    CONN_h.menus.m_analyses_00{30}=conn_menu('pushbutton',boffset+[.34,.31,.02,.13],'','<','move elements between ''Seeds/Sources'' and ''all ROIs'' lists', 'conn(''gui_analyses'',0);');
                    ...
                    CONN_h.menus.m_analyses_00{5}=conn_menu('popup',boffset+[.38,.11,.15,.04],'',{'no frequency decomposition','frequency decomposition'},'Number of frequency bands for spectral decomposition of selected seed/source timeseries (''no decomposition'' for single-band covering entire band-pass filtered data)','conn(''gui_analyses'',5);');
                    ...
                    CONN_h.menus.m_analyses_00{22}=[...%uicontrol('style','frame','units','norm','position',boffset+[.405,.30,.125,.33],'foregroundcolor',CONN_gui.backgroundcolorA,'backgroundcolor',CONN_gui.backgroundcolorA),...
                        uicontrol('style','frame','units','norm','position',boffset+[.136,.08,.379,.23],'foregroundcolor',CONN_gui.backgroundcolorA,'backgroundcolor',CONN_gui.backgroundcolorA)];
...
%                         end
            ...
            elseif state(1)==3 % VOXEL-TO-VOXEL
                ...
                if nargin<2,
                    ...
                    conn_menu('frame',boffset+[.215,.35,.25,.425],'Connectome analyses');%'FC ANALYSIS OPTIONS');
                    if isfield(CONN_x.vvAnalyses,'name')&&~isempty(CONN_x.vvAnalyses(CONN_x.vvAnalysis).name)
                        ...
                        CONN_h.menus.m_analyses_00{20}=conn_menu('popupbigblue',boffset+[.216,.73,.25,.05],'',txt(CONN_x.vvAnalysis,:),'<HTML>Analysis name <br/> - Select existing first-level analysis name to edit its properties <br/> - select <i>new/rename/delete</i> to define a new set of first-level analyses within this project, or the rename or delete the selected analysis</HTML>','conn(''gui_analyses'',20);');
                    ...
                    if any(ok), set(CONN_h.menus.m_analyses_00{16},'value',idx(find(ok,1))); end 
...
%                                 end
                ...
                if nargin<2, 
                    ...
                    if isfield(CONN_x.dynAnalyses,'name')&&~isempty(CONN_x.dynAnalyses(CONN_x.dynAnalysis).name)
                        ...
                        CONN_h.menus.m_analyses_00{20}=conn_menu('popupbigblue',boffset+[.096,.78,.38,.05],'',txt(CONN_x.dynAnalysis,:),'<HTML>Analysis name <br/> - Select existing first-level analysis name to edit its properties <br/> - select <i>new/rename/delete</i> to define a new set of first-level analyses within this project, or the rename or delete the selected analysis</HTML>','conn(''gui_analyses'',20);');
                    ...
                    CONN_h.menus.m_analyses_00{11}=[]; [CONN_h.menus.m_analyses_00{11}(1),CONN_h.menus.m_analyses_00{11}(2)]=conn_menu('checkbox',boffset+[.105,.25,.02,.025],'Export modulatory timeseries','','(optional) Export the estimated modulatory timeseries as first-level covariates for additional analyses','conn(''gui_analyses'',11);');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%			
        ...
        case {'gui_results','gui_results_s2v','gui_results_r2r','gui_results_v2v','gui_results_dyn','gui_results_dyn_spatial','gui_results_dyn_temporal','gui_results_dyn_summary','gui_results_ica','gui_results_ica_spatial','gui_results_ica_temporal','gui_results_ica_summary'}
            ...
            if ~isfield(CONN_x.Analyses(ianalysis),'name'),CONN_x.Analyses(ianalysis).name='SBC_01'; end
            ...
            if nargin<2,
                ...
                elseif state==5
                    ...
                    if state==2||state==3,
                        ...
                        CONN_h.menus.m_results_00{32}=conn_menu('popup2big',boffset+[pos(1)+.02,.87,.28,.045],'',strstr3,'<HTML>Preview options:<br/> - select <i>full model</i> to preview the full General Linear Model results for the selected between-subjects/conditions/sources contrasts<br/> - select <i>individual contrasts</i> for repeated measures or multivariate analyses to display separately each individual univariate contrast<br/><br/>Click <i>Resuts Explorer</i> for additional thresholding, statistics, and display options</HTML>','conn(''gui_results'',32);');
                        ...
                        CONN_h.menus.m_results_00{44}=conn_menu('pushbutton2',boffset+[pos(1)-.03,pos(2)-.31,.07,.045],'','plot subjects','displays connectivity measures for each subject','conn(''gui_results'',44);');
...
% 						model=1;modelroi=1;
                    ...
                    case 44
                        ...
                        answ=conn_questdlg('','Plot options',str{[1 2 1]});
                        if isempty(answ), 
                        elseif isequal(answ,str{1})
                            if ~isempty(CONN_h.menus.m_results.y.data)&&~conn_surf_dimscheck(CONN_h.menus.m_results.Y(1).dim),
                                ...
                                if numel(files)==1, select=1;
                                    ...
                                    answ=listdlg('name','','PromptString','Select condition/source','ListString',reshape(CONN_h.menus.m_results.design.Ytitle,1,[]),'SelectionMode','sigle','ListSize',[200 200]);
                                    if isempty(answ), return; end
                                    select=answ;
                                end
