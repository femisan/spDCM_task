...
if isempty(FS_folder) % extracts from single surface file (assumes all files in MNI space)
    ...
    if refinfoinit
        if isstruct(FS_folder)
            ...
            if strcmp(temp2,'mri')||strcmp(temp2,'anat'), FS_folder=temp1; 
            end
            a.vox2ras1=vol.mat;
            ...
            ivalid=find(valid,1);
            if ~isempty(regexp(names{ivalid},'\.mgh\s*$|\.mgz\s*$')), names{ivalid}=conn_mgh2nii(names{ivalid}); end
            vol=spm_vol(fullfile(FS_folder,'mri',names{ivalid}));
            a.vox2ras1=vol.mat;
            a.volsize=vol.dim([2 1 3]);
            a.volres = sqrt(sum(vol.mat(:,1:3).^2,1));
            a.vox2ras0=conn_freesurfer_vox2ras_1to0(vol.mat);
