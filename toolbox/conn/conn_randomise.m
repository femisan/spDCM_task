...
if nargin<11||isempty(datatype), datatype='none'; end % 'none'|'matrix'|'surface'|volumecoords
    ...
    try
        ...
        for n1=1:numel(newPthr),
            ...
            if ~isempty(tidx2), 
                ...
                fprintf('%d runs (%d randomization samples) already matching threshold values %s. Adding %d new randomizaton samples\n',numel(tidx2),ndone,mat2str([newPthr(n1) newPthr_type(n1) newPthr_side(n1)]),niters); 
            end
        end
...
elseif isdisplay==0, disp('Updating non-parametric statistics. Please wait'); 
end
...
for niter=1:niters
    ...
    if ~univariate||any(Pthr_type(thridx)>1),
        p=nan(size(f));
