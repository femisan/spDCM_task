...
switch(lower(option))
    ...
    case 'get'
        ...
        switch(lower(varargin{1}))
            ...
            case 'l2covariates',
                ...
                if numel(varargin)>1,
                    icov=find(ismember(names,cellstr(varargin{2})));
                    if isempty(icov), icov=find(cellfun('length',regexp(names,varargin{2}))); end
                    if numel(icov)==1,
                        names=names{icov};
                        other=other{icov};
                        files=files(:,icov);
    ...
    case 'preprocessing'
        ...
        Fields={'functionals','structurals','nsubjects','RT','unwarp_functionals','coregsource_functionals','localcopy'}; % send these to Setup
