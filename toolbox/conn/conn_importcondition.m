...
options=struct('breakconditionsbysession',false,'deleteall',false,'dogui',false,'subjects',1:CONN_x.Setup.nsubjects);
...
switch(filetype)
    ...
    case {3,4}, % BIDS one file per subject/session
        ...
        for isub=1:numel(filename)
            nsub=options.subjects(isub);
            if ~iscell(filename{isub}), filename{isub}={filename{isub}}; allsess=true;
            else allsess=false; 
            end
